...
The `home-routes` handler in the `account-api-v2.routes.home` namespace
...
The `home-page` function will in turn call the `account-api-v2.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `account-api-v2.handler` namespace:
...
Request middleware functions are located under the `account-api-v2.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `account-api-v2.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `account-api-v2.core` in some other namespace.
